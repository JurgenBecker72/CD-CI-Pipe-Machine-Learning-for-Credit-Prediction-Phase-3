...
# Load the applicant payload from scripts/smoke_payload.json. That fixture is
# generated by scripts/refresh_smoke_payload.py from a real warehouse row, so
# every feature the model was trained on is present. Sending a partial payload
# triggers fillna(0) in the serving layer and produces a degenerate score --
# we want a realistic baseline.
$payloadPath = Join-Path $PSScriptRoot "smoke_payload.json"
if (-not (Test-Path $payloadPath)) {
    Write-Log ""
    Write-Log "ERROR: $payloadPath not found."
    Write-Log "Generate it from the warehouse first:"
    Write-Log "  uv run python scripts/refresh_smoke_payload.py"
    exit 1
}
